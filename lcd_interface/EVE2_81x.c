...
//
// In pursuit of the common goal of simplicity and understandability I find that I am unable to
...
// Call this function once at power-up to reset and initialize the Eve chip
...
//  Log("Chip ID = 0x%04x%04x\n", ValH, ValL);
  ...
  wr8(REG_PWM_DUTY + RAM_REG, 128);                 // Backlight PWM duty (off)
...
void Eve_Reset(void)
{
  P4OUT &= ~BIT5;          //Set RST LOW (TRUE)
  __delay_cycles(1000000);  //wait
  P4OUT |= BIT5;            //Set RST HIGH
  __delay_cycles(1000000);  //wait
}
...
uint32_t rd32(uint32_t address)
{
  ...
  SPI_ReadBuffer(buf, 5);
  
  SPI_Disable();
  
  Data32 = buf[1] + ((uint32_t)buf[2] << 8) + ((uint32_t)buf[3] << 16) + ((uint32_t)buf[4] << 24);
  return (Data32);  
}
...
uint16_t rd16(uint32_t address)
{
  ...
  SPI_ReadBuffer(buf, 3);
  
  SPI_Disable();
  
  uint16_t Data16 = buf[1] + ((uint16_t)buf[2] << 8);
  return (Data16);  
}
...
uint8_t Cmd_READ_REG_ID(void)
{
  ...
  SPI_ReadBuffer(readData, 2);       // There was a dummy read of the first byte in there
  //__delay_cycles(50);
  SPI_Disable();
  
  if (readData[1] == 0x7C)           // FT81x Datasheet section 5.1, Table 5-2. Return value always 0x7C
  {
    printf("\nGood ID: 0x%02x\n", readData[1]);
    return 1;
  }
...
void Cmd_Keys(uint16_t x0, uint16_t y0, uint16_t w0, uint16_t h0, uint16_t font, uint16_t options, const char* str){
    uint16_t DataPtr, LoopCount, StrPtr;
...
void Wait4CoProFIFOEmpty(void)
{
  ...
  do
  {
    ...
    if(ReadReg == 0xFFF)
    {
      ...
      wr8(REG_CPU_RESET + RAM_REG, 0);
      wr32(REG_COPRO_PATCH_PTR + RAM_REG, Patch_Add);
      __delay_cycles(250);  // we already saw one error message and we don't need to see then 1000 times a second
    }
...
uint32_t WriteBlockRAM(uint32_t Addr, const uint8_t *buff, uint32_t count)
{
  uint8_t index;
  uint32_t WriteAddress = Addr;  // I want to return the value instead of modifying the variable in place
